...
def test_get_available_roles_with_data(app, db):
    application = Application(
        ...
        available_roles=json.dumps(['admin', 'engineer', 'viewer']),
        default_role='viewer',
    )
    db.session.add(application)
    db.session.commit()
    assert application.get_available_roles() == ['admin', 'engineer', 'viewer']
    assert application.default_role == 'viewer'


def test_set_available_roles(app, sample_app, db):
    sample_app.set_available_roles(['admin', 'tech'])
    db.session.commit()
    assert sample_app.get_available_roles() == ['admin', 'tech']


def test_set_available_roles_clears(app, sample_app, db):
    sample_app.set_available_roles(['admin'])
    db.session.commit()
...
def test_validate_token_returns_default_role(client, app, db, normal_user):
    ...
    application = Application(
        app_code='heatsim', app_name='HeatSim',
        internal_url='http://localhost:5002',
    ...
    resp = client.post('/api/validate-token',
        data=json.dumps({'token': token, 'app_code': 'heatsim'}),
        content_type='application/json')
    data = resp.get_json()
    assert data['valid'] is True
    assert data['user']['role'] == 'engineer'
...
def test_grant_permission_with_role(logged_in_client, db, normal_user):
    application = Application(
        app_code='roleapp', app_name='Role App',
        internal_url='http://localhost:9000',
        available_roles=json.dumps(['admin', 'operator']),
...
def test_set_role(logged_in_client, db, normal_user):
    ...
    db.session.add(application)
    db.session.flush()
    perm = UserPermission(user_id=normal_user.id, app_id=application.id, role='viewer')
    db.session.add(perm)
    db.session.commit()
...
def test_permission_matrix_shows_roles(logged_in_client, db, normal_user):
    ...
    db.session.add(application)
    db.session.flush()
    perm = UserPermission(user_id=normal_user.id, app_id=application.id, role='admin')
    db.session.add(perm)
    db.session.commit()
