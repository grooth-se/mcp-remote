...
class TensileTestApp:
    ...
    def save_to_database(self):
        """Save current test data to database."""
        ...
        try:
            ...
            raw_data = {
                ...
                'extension': self.current_data.extension,
                'source_file': self.file_var.get(),
                'num_points': len(self.current_data.time),
            }
            db.save_raw_data(test_id, 'main', raw_data)

            # 4. Save results if available
            if self.current_results:
                ...
                for param_name, result in self.current_results.items():
                    ...
                    elif param_name in ['yield_type', 'strain_source']:
                        # Store as extra data in a special result
                        results_list.append({
                            'parameter_name': param_name,
                            'value': 0,
                            'uncertainty': 0,
                            'unit': str(result),
                            'extra_data': {'string_value': str(result)},
                            'is_valid': True,
                        })
            ...
            if self.stress is not None and self.strain is not None:
                # Generate plot image
                buf = io.BytesIO()
                self.fig.savefig(buf, format='png', dpi=150, bbox_inches='tight')
                buf.seek(0)
                plot_data = buf.read()
                db.save_plot(test_id, plot_data, description='Stress-Strain Curve')

            self._update_status(f"Saved to database: {cert_num}")
            messagebox.showinfo("Success", f"Test saved to database:\n{cert_num}")
    ...
    def load_from_database(self):
        ...
        try:
            ...
            cert_numbers = db.get_certificate_numbers_list(test_type='TENSILE')

            if not cert_numbers:
                messagebox.showinfo("Info", "No tensile tests found in database.")
                return
            ...
            def refresh_list(search_term=''):
                ...
                if search_term:
                    tests = db.search_tests(search_term, test_type='TENSILE')
                else:
                    tests = db.get_all_tests(test_type='TENSILE')
    ...
    def _load_test_from_db(self, certificate_number: str):
        """Load specific test from database."""
        try:
            from utils.database.test_data_db import TestDataDatabase
            ...
            if geometry:
                # Set specimen type
                spec_type = geometry.get('specimen_type', 'round')
            ...
            if raw_data:
                # Create a data container similar to MTSTestData
                @dataclass
                ...
                self.current_data = LoadedTestData(
                    time=raw_data.get('time') if raw_data.get('time') is not None else np.array([]),
                    force=raw_data.get('force') if raw_data.get('force') is not None else np.array([]),
                    displacement=raw_data.get('displacement') if raw_data.get('displacement') is not None else np.array([]),
                    extension=raw_data.get('extension') if raw_data.get('extension') is not None else np.array([]),
                    file_path=raw_data.get('source_file', '')
                )
                self.file_var.set(raw_data.get('source_file', 'Loaded from database'))
            ...
            if results:
                ...
                for r in results:
                    ...
                    if param in ['yield_type', 'strain_source']:
                        # Extract string value
                        extra = r.get('extra_data', {})
                        if extra and 'string_value' in extra:
                            self.current_results[param] = extra['string_value']
                        else:
                            self.current_results[param] = r.get('unit', '')
                    else:
            ...
            # Run analysis to regenerate plot and stress/strain arrays
            if self.current_data is not None and len(self.current_data.force) > 0:
                self.run_analysis()
