...
def index():
    ...
    templates = query.order_by(
        HeatTreatmentTemplate.use_count.desc(), HeatTreatmentTemplate.name
    ).all()
...
def new():
    ...
    if form.validate_on_submit():
        ...
        flash(
            f'Template "{template.name}" created. Configure the heat treatment settings.', "success"
        )
...
def api_list():
    ...
    templates = (
        HeatTreatmentTemplate.query.filter(
            or_(
                HeatTreatmentTemplate.user_id == current_user.id,
                HeatTreatmentTemplate.is_public == True,
            )
        )
        .order_by(HeatTreatmentTemplate.use_count.desc(), HeatTreatmentTemplate.name)
        .all()
    )
...
def _parse_ht_config_from_form(form_data):
    ...
    return {
        ...
        "quenching": {
            ...
            "htc_override": get_float("quenching_htc_override")
            if form_data.get("quenching_htc_override")
            else None,
