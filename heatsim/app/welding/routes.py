...
def index():
    ...
    projects = (
        WeldProject.query.filter_by(user_id=current_user.id)
        .order_by(WeldProject.created_at.desc())
        .all()
    )
...
def plot(id, plot_type):
    ...
    if plot_type == "thermal_cycles":
        ...
        img_data = viz.create_thermal_cycle_plot(
            thermal_results, title=f"{project.name} - Thermal Cycles"
        )

    elif plot_type == "cooling_rates":
        cooling_results = [r for r in results if r.result_type == RESULT_COOLING_RATE]
        img_data = viz.create_cct_overlay_plot(
            cooling_results, title=f"{project.name} - Cooling Rates"
        )

    elif plot_type == "phases":
        thermal_results = [r for r in results if r.result_type == RESULT_THERMAL_CYCLE]
        img_data = viz.create_phase_fraction_plot(
            thermal_results, title=f"{project.name} - Phase Fractions"
        )
...
def _get_goldak_data(project: WeldProject, form_data=None) -> dict:
    ...
    try:
        ...
        solver = GoldakSolver.from_weld_project(
            ...
            a_f_override=a_f_override,
            a_r_override=a_r_override,
        )
        result = solver.solve()
        return result.to_dict()
    except Exception as e:
...
def goldak_analysis(id):
    ...
    pool_estimate = estimate_pool_params(
        project.default_heat_input or 1.5, project.process_type or "mig_mag"
    )
...
def goldak_multipass(id):
    ...
    if form.validate_on_submit():
        ...
        project.progress_message = (
            f"goldak:{form.grid_resolution.data}:{str(form.compare_methods.data).lower()}"
        )
        db.session.commit()
    ...
    is_goldak_running = project.status in (STATUS_QUEUED, STATUS_RUNNING) and (
        project.progress_message or ""
    ).startswith(("goldak", "Pass ", "Initializing Goldak"))
...
def goldak_multipass_status(id):
    ...
    queue_position = (
        get_queue_position("weld", project.id) if project.status == STATUS_QUEUED else None
    )
