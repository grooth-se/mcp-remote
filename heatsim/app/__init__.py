...
def create_app(config_name="default"):
        ...
        try:
            ...
            if mat_engine:
                with mat_engine.connect() as conn:
                    ...
                    if total > 0:
                        conn.commit()
                        app.logger.info(f"Reset {total} stuck job(s) to failed")
        except Exception:
    ...
    def check_maintenance_mode():
        ...
        if endpoint.startswith("auth.") or endpoint.startswith("admin.") or endpoint == "static":
            return None
