...
from wtforms import FloatField, HiddenField, SelectField, SubmitField, TextAreaField
...
class TTTParametersForm(FlaskForm):
    ...
    grain_size_astm = FloatField(
        "Grain Size (ASTM)", validators=[Optional(), NumberRange(1, 14)], default=8.0
    )
...
class JMAKParametersForm(FlaskForm):
    ...
    n_value = FloatField("Avrami Exponent (n)", validators=[DataRequired(), NumberRange(0.1, 10)])
    b_model_type = SelectField(
        "b(T) Model",
        choices=[
            ("gaussian", "Gaussian"),
            ("arrhenius", "Arrhenius"),
...
class MartensiteForm(FlaskForm):
    ...
    alpha_m = FloatField(
        "Alpha (1/K)", validators=[DataRequired(), NumberRange(0.001, 0.1)], default=0.011
    )
...
class CalibrationUploadForm(FlaskForm):
    ...
    csv_file = FileField(
        "CSV File", validators=[DataRequired(), FileAllowed(["csv"], "CSV files only")]
    )
