...
def edit_parameters(grade_id):
    ...
    return render_template("ttt_cct/edit_parameters.html", grade=grade, ttt=ttt, form=form)
...
def edit_jmak(grade_id, phase):
    ...
    return render_template("ttt_cct/edit_jmak.html", grade=grade, phase=phase, form=form)
...
def edit_martensite(grade_id):
    ...
    return render_template("ttt_cct/edit_martensite.html", grade=grade, form=form)
...
def calibrate(grade_id):
            ...
            try:
                point = {
                    ...
                    "fraction_transformed": float(
                        row.get("fraction", row.get("fraction_transformed", 0))
                    ),
                }
    ...
    return render_template("ttt_cct/calibration.html", grade=grade, form=form)
...
def ttt_plot(grade_id):
    ...
    plot_data = visualization.create_ttt_plot(
        curves=curves, transformation_temps=trans_temps, title=f"TTT Diagram - {grade.designation}"
    )
