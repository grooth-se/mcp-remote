...
from app.models import MeasuredData, Simulation, SteelGrade
...
def dashboard():
    ...
    recent_simulations = user_sims.order_by(Simulation.created_at.desc()).limit(10).all()

    # Simulations this week
    week_ago = datetime.utcnow() - timedelta(days=7)
    stats["this_week"] = user_sims.filter(Simulation.created_at >= week_ago).count()

    # Most used steel grades (top 5)
    top_grades = (
        db.session.query(SteelGrade.designation, func.count(Simulation.id).label("count"))
        .join(Simulation)
    ...
    sims_with_measured = (
        db.session.query(func.count(func.distinct(MeasuredData.simulation_id)))
        .filter(
    ...
    for i in range(7, -1, -1):
        ...
        count = user_sims.filter(
            Simulation.created_at >= week_start, Simulation.created_at < week_end
        ).count()
        weekly_counts.append({"week": week_end.strftime("%m/%d"), "count": count})

    # t8/5 range insights
    t85_values = (
        db.session.query(SimulationResult.t_800_500)
        .join(Simulation)
        .filter(
