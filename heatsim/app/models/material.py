...
class SteelGrade(db.Model):
    ...
    properties = db.relationship(
        "MaterialProperty", backref="steel_grade", lazy="dynamic", cascade="all, delete-orphan"
    )
    phase_diagrams = db.relationship(
        "PhaseDiagram", backref="steel_grade", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class PhaseDiagram(db.Model):
    ...
    __table_args__ = (db.Index("ix_phase_diagrams_steel_grade", "steel_grade_id"),)
...
class PhaseProperty(db.Model):
    ...
    steel_grade = db.relationship(
        "SteelGrade",
        backref=db.backref("phase_properties", lazy="dynamic", cascade="all, delete-orphan"),
    )
    ...
    def get_expansion_at_temperature(self, temperature: float) -> float | None:
        ...
        interp = interpolate.interp1d(
            temps, values, kind="linear", bounds_error=False, fill_value=(values[0], values[-1])
        )
...
class SteelComposition(db.Model):
    ...
    # Relationship
    steel_grade = db.relationship(
        "SteelGrade", backref=db.backref("composition", uselist=False, cascade="all, delete-orphan")
    )

    __table_args__ = (db.Index("ix_steel_compositions_steel_grade", "steel_grade_id"),)
    ...
    def carbon_equivalent_cen(self) -> float:
        ...
        alloy_term = Si / 24 + Mn / 6 + Cu / 15 + Ni / 20 + (Cr + Mo + Nb + V) / 5 + 5 * B
