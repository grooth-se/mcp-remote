...
PROCESS_TYPES = [
    ...
    PROCESS_QUENCH_OIL,
    PROCESS_QUENCH_POLYMER,
    PROCESS_QUENCH_AIR,
    PROCESS_TEMPERING,
    PROCESS_NORMALIZING,
    PROCESS_STRESS_RELIEF,
    PROCESS_CUSTOM,
]
...
class Simulation(db.Model):
    ...
    results = db.relationship(
        "SimulationResult", backref="simulation", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    def get_phases_summary(self) -> list[str]:
        ...
        if self.has_heating:
            ...
            phases.append(
                f"Heating: {h.get('target_temperature', 850)}°C for {h.get('hold_time', 0)}min"
            )
        ...
        if self.has_tempering:
            ...
            phases.append(
                f"Tempering: {t.get('temperature', 550)}°C for {t.get('hold_time', 60)}min"
            )
...
class SimulationResult(db.Model):
    ...
    __table_args__ = (db.Index("ix_simulation_results_sim", "simulation_id"),)
...
class HeatTreatmentTemplate(db.Model):
    ...
    def geometry_dict(self) -> dict:
        ...
        try:
            return (
                json.loads(self.suggested_geometry_config) if self.suggested_geometry_config else {}
            )
        except json.JSONDecodeError:
            return {}
