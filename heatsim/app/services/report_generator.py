...
class SimulationReportGenerator:
    ...
    def _add_header(self) -> None:
        ...
        # Title
        title = self.doc.add_heading("Heat Treatment Simulation Report", 0)
    ...
    def _add_geometry_section(self) -> None:
        ...
        if geom_type == "cylinder":
            ...
            self._add_table_row(
                table, "Outer Diameter", f"{geom.get('outer_diameter', 0) * 1000:.1f} mm"
            )
            self._add_table_row(
                table, "Inner Diameter", f"{geom.get('inner_diameter', 0) * 1000:.1f} mm"
            )
            self._add_table_row(table, "Length", f"{geom.get('length', 0) * 1000:.1f} mm")
        elif geom_type == "ring":
            self._add_table_row(
                table, "Inner Radius", f"{geom.get('inner_radius', 0) * 1000:.1f} mm"
            )
            self._add_table_row(
                table, "Outer Radius", f"{geom.get('outer_radius', 0) * 1000:.1f} mm"
            )
            self._add_table_row(table, "Length", f"{geom.get('length', 0) * 1000:.1f} mm")
        elif geom_type == "cad":
            ...
            if cad:
                ...
                self._add_table_row(
                    table, "Surface Area", f"{cad.get('surface_area', 0) * 1e4:.1f} cm²"
                )
                self._add_table_row(
                    table,
                    "Characteristic Length",
                    f"{cad.get('characteristic_length', 0) * 1000:.2f} mm",
                )
                self._add_table_row(
                    table, "Equivalent Type", self.sim.cad_equivalent_type or "auto"
                )
    ...
    def _add_heat_treatment_section(self) -> None:
        ...
        if heating.get("enabled", False):
            ...
            self._add_table_row(
                table, "Initial Temperature", f"{heating.get('initial_temperature', 25):.0f} °C"
            )
            self._add_table_row(
                table, "Target Temperature", f"{heating.get('target_temperature', 850):.0f} °C"
            )
            self._add_table_row(table, "Hold Time", f"{heating.get('hold_time', 60):.0f} min")
            self._add_table_row(
                table, "Furnace Atmosphere", heating.get("furnace_atmosphere", "air").title()
            )
            self._add_table_row(table, "Furnace HTC", f"{heating.get('furnace_htc', 25):.0f} W/m²K")
            if heating.get("use_radiation", True):
                self._add_table_row(
                    table, "Emissivity", f"{heating.get('furnace_emissivity', 0.85):.2f}"
                )
            if heating.get("cold_furnace", False):
                self._add_table_row(
                    table,
                    "Cold Furnace Start",
                    f"{heating.get('furnace_start_temperature', 25):.0f} °C",
                )
                self._add_table_row(
                    table, "Ramp Rate", f"{heating.get('furnace_ramp_rate', 5):.1f} °C/min"
                )
        ...
        if transfer.get("enabled", False):
            ...
            self._add_table_row(
                table, "Ambient Temperature", f"{transfer.get('ambient_temperature', 25):.0f} °C"
            )
        ...
        if quenching:
            ...
            self._add_table_row(
                table, "Media Temperature", f"{quenching.get('media_temperature', 25):.0f} °C"
            )
            agitation = quenching.get("agitation", "moderate")
            self._add_table_row(
                table, "Agitation", AGITATION_LABELS.get(agitation, agitation.title())
            )
        ...
        if tempering.get("enabled", False):
            ...
            self._add_table_row(
                table, "Cooling Method", tempering.get("cooling_method", "air").title()
            )
    ...
    def _add_results_summary(self) -> None:
        ...
        if self.sim.completed_at:
            self._add_table_row(
                table, "Completed", self.sim.completed_at.strftime("%Y-%m-%d %H:%M:%S")
            )
        ...
        if data and data.get("hardness_hv"):
            ...
            if data.get("uts_mpa"):
                ...
                for label, key in positions:
                    ...
                    row.cells[0].text = label
                    row.cells[1].text = (
                        f"{data['uts_mpa'].get(key, 0):.0f}" if data["uts_mpa"].get(key) else "-"
                    )
                    row.cells[2].text = (
                        f"{data['ys_mpa'].get(key, 0):.0f}" if data["ys_mpa"].get(key) else "-"
                    )
                    row.cells[3].text = (
                        f"{data.get('elongation_pct', {}).get(key, 0):.1f}"
                        if data.get("elongation_pct", {}).get(key)
                        else "-"
                    )
...
class SimulationPDFReportGenerator:
    ...
    def _add_header(self) -> None:
        ...
        self.pdf.cell(
            0, 15, "Heat Treatment Simulation Report", align="C", new_x="LMARGIN", new_y="NEXT"
        )
        ...
        self._add_info_table(
            [
                (
                    "Steel Grade",
                    self.sim.steel_grade.designation if self.sim.steel_grade else "N/A",
                ),
                (
                    "Data Source",
                    self.sim.steel_grade.data_source if self.sim.steel_grade else "N/A",
                ),
                ("Status", self.sim.status.title()),
                ("Generated", datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")),
            ]
        ...
        if phases:
            ...
            for phase, fraction in phases.items():
                if fraction > 0.01:
                    ...
                    self.pdf.cell(
                        0, 7, f"{fraction * 100:.1f}", border=1, new_x="LMARGIN", new_y="NEXT"
                    )
        ...
        if data and data.get("hardness_hv"):
            ...
            if data.get("carbon_equivalent"):
                self.pdf.cell(
                    0,
                    7,
                    f"Carbon Equivalent CE(IIW): {data['carbon_equivalent']:.3f}",
                    new_x="LMARGIN",
                    new_y="NEXT",
                )
            if data.get("ideal_diameter"):
                self.pdf.cell(
                    0,
                    7,
                    f"Ideal Diameter DI: {data['ideal_diameter']:.2f} in",
                    new_x="LMARGIN",
                    new_y="NEXT",
                )
            ...
            if data.get("uts_mpa"):
                ...
                self.pdf.set_font("Helvetica", "B", 12)
                self.pdf.cell(
                    0, 8, "Estimated Mechanical Properties", new_x="LMARGIN", new_y="NEXT"
                )
