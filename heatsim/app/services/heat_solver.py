...
from app.services.boundary_conditions import (
    ...
    create_heating_bc,
    create_quench_bc,
    create_ramping_heating_bc,
    create_tempering_bc,
    create_transfer_bc,
)
from app.services.geometry import Cylinder, GeometryBase, Ring
...
class PhaseConfig:
    ...
    end_condition: str = (
        "time"  # 'time', 'temperature', 'equilibrium', 'rate_threshold', 'center_offset'
    )
    ...
    def from_quenching_config(cls, config: dict) -> "PhaseConfig":
        ...
        # Default True keeps legacy configs (missing the key) behaving as before.
        if not config.get("enabled", True):
            return cls(name="quenching", enabled=False)
...
class HeatSolver:
    ...
    def __init__(
        ...
        inner_bc: BoundaryCondition | None = None,
        ...
        self._k_evaluator: PropertyEvaluator | None = None
        self._cp_evaluator: PropertyEvaluator | None = None
    ...
    def solve(
        self, initial_temp: float, progress_callback: Callable[[float], None] | None = None
    ) -> SolverResult:
    ...
    def solve_phase(
        ...
        has_approached_target = (
            initial_center_temp <= phase_config.target_temperature + 50
            if phase_config.target_temperature
            else True
        )
        ...
        while t < max_phase_time:
            ...
            if phase_config.end_condition == "equilibrium":
                ...
                if t - last_rate_check_time >= rate_check_interval:
                    surface_rate = (
                        abs(T[-1] - prev_surface_temp) / rate_check_interval * 3600
                    )  # °C/hr
                    prev_surface_temp = T[-1]
    ...
    def _time_step(self, T: np.ndarray, r: np.ndarray, dr: float, dt: float) -> np.ndarray:
    ...
    def _build_cylindrical_system(
        ...
        for i in range(1, n - 1):
            ...
            rhs[i] = T[i] + (1 - theta) * (a_w * (T[i - 1] - T[i]) + a_e * (T[i + 1] - T[i]))
...
class MultiPhaseHeatSolver:
    ...
    def __init__(self, geometry: GeometryBase, config: SolverConfig | None = None):
    ...
    def configure_from_ht_config(self, ht_config: dict):
        ...
        # Quenching phase — default-on for backward compat with legacy configs
        # that omit the `enabled` key. Set explicitly to False to skip.
        quenching = ht_config.get("quenching", {})
        if quenching.get("enabled", True):
