...
def create_cooling_curve_plot(
    ...
    quarter_temp: np.ndarray | None = None,
    title: str = "Cooling Curves",
    transformation_temps: dict | None = None,
) -> bytes:
...
def create_phase_fraction_plot(phases: dict, title: str = "Final Phase Fractions") -> bytes:
    ...
    for bar, frac in zip(bars, fractions, strict=False):
...
def create_heat_treatment_cycle_plot(
    ...
    furnace_temps: list[dict] | None = None,
) -> bytes:
    ...
    if measured_data:
        for i, data in enumerate(measured_data):
            ...
            ax.plot(m_times, m_temps, color=color, linewidth=1.5, label=f"Meas: {name}", alpha=0.9)
...
def create_dTdt_vs_time_plot(
    ...
    for key in ["center", "one_third", "two_thirds", "surface"]:
        ...
        ax.plot(
            time_mid, dTdt, color=FOUR_POINT_COLORS[key], linewidth=2, label=FOUR_POINT_LABELS[key]
        )
...
def create_dTdt_vs_temperature_plot(
    ...
    for key in ["center", "one_third", "two_thirds", "surface"]:
        ...
        ax.plot(
            T_mid, dTdt, color=FOUR_POINT_COLORS[key], linewidth=2, label=FOUR_POINT_LABELS[key]
        )
...
def create_measured_tc_plot(
    measured_data: list[dict], title: str = "Measured Temperature vs Time"
) -> bytes:
...
def filter_outliers_percentile(
    data: np.ndarray, lower_pct: float = 1, upper_pct: float = 99
) -> tuple:
...
def create_measured_dtdt_plot(
    ...
    if filter_outliers and len(all_dTdt) > 0:
        _, y_min, y_max = filter_outliers_percentile(
            all_dTdt, outlier_percentile, 100 - outlier_percentile
        )
        # Add some padding
        y_range = y_max - y_min
        y_min -= 0.1 * y_range
...
def create_hardness_profile_plot(
    hardness_result, title: str = "Predicted Hardness Profile"
) -> bytes:
    ...
    has_tempered = bool(
        getattr(hardness_result, "tempered_hardness_hv", None)
    ...
    if has_tempered:
        ...
        for bar, hv in zip(bars_q, hv_values, strict=False):
            height = bar.get_height()
            ax.annotate(
                f"{hv:.0f}",
                xy=(bar.get_x() + bar.get_width() / 2, height),
        ...
        for bar, hv in zip(bars_t, tempered_hv, strict=False):
            height = bar.get_height()
            ax.annotate(
                f"{hv:.0f}",
                xy=(bar.get_x() + bar.get_width() / 2, height),
        ...
        for i, (bar, hv, hrc) in enumerate(zip(bars, hv_values, hrc_values, strict=False)):
            height = bar.get_height()
            ax.annotate(
                f"{hv:.0f} HV",
                xy=(bar.get_x() + bar.get_width() / 2, height),
    ...
    t8_5_text = "t8/5: "
    t8_5_parts = []
    for p, label in zip(positions, labels, strict=False):
        t8_5 = hardness_result.t8_5_values.get(p)
        if t8_5:
            t8_5_parts.append(f"{label}={t8_5:.1f}s")
    t8_5_text += ", ".join(t8_5_parts)

    ax.text(0.5, -0.12, t8_5_text, transform=ax.transAxes, fontsize=9, ha="center", color="gray")
...
def create_comparison_overlay_plot(
    sim_data: list[dict], title: str = "Temperature Comparison"
) -> bytes:
...
def create_cct_overlay_plot(
    ...
    if curves:
        for phase, phase_curves in curves.items():
            ...
            has_start = (
                "start" in phase_curves and phase_curves["start"] and len(phase_curves["start"]) > 1
            )
            has_finish = (
                "finish" in phase_curves
                and phase_curves["finish"]
                and len(phase_curves["finish"]) > 1
            )
            ...
            if has_start and has_finish:
                ...
                if t_hi > t_lo:
                    ...
                    start_times = np.interp(
                        common_temps, start_data[:, 1][::-1], start_data[:, 0][::-1]
                    )
                    finish_times = np.interp(
                        common_temps, finish_data[:, 1][::-1], finish_data[:, 0][::-1]
                    )
                    ax.fill_betweenx(
                        common_temps,
                        start_times,
    ...
    if ms is not None and mf is not None:
        ax.axhspan(mf, ms, alpha=0.10, color="#d62728", zorder=1, label="Martensite region")
    ...
    if curves:
        for phase, phase_curves in curves.items():
            ...
            has_start = (
                "start" in phase_curves
                and phase_curves["start"]
                and len(phase_curves.get("start", [])) > 1
            )
            has_finish = (
                "finish" in phase_curves
                and phase_curves["finish"]
                and len(phase_curves.get("finish", [])) > 1
            )
    ...
    temp_lines = [
        (
            "Ac3",
            transformation_temps.get("Ac3") or transformation_temps.get("Ae3"),
            "#9467bd",
            "Ac3",
        ),
        (
            "Ac1",
            transformation_temps.get("Ac1") or transformation_temps.get("Ae1"),
            "#8c564b",
            "Ac1",
        ),
    ...
    if temperatures.size > 0:
        if temperatures.ndim == 1:
            ...
            ax.plot(
                times[valid_mask],
                temperatures[valid_mask],
                "k-",
                linewidth=3,
                label="Cooling curve (center)",
                zorder=10,
            )
        else:
            ...
            for i in range(min(n_pos, 4)):
                ...
                ax.plot(
                    times[valid_mask],
                    temperatures[valid_mask, i],
                    color=pos_colors[i],
                    linewidth=2.5,
                    label=f"{pos_labels[i]}",
                    zorder=10,
                )
...
def create_ttt_plot(
    curves: dict, transformation_temps: dict | None = None, title: str = "TTT Diagram"
) -> bytes:
    ...
    for phase, phase_curves in curves.items():
        ...
        has_finish = (
            "finish" in phase_curves and phase_curves["finish"] and len(phase_curves["finish"]) > 1
        )
...
def create_ttt_overlay_plot(
    ...
    return create_cct_overlay_plot(times, temperatures, transformation_temps, curves, title=title)


def create_jominy_curve_plot(
    distances_mm: list[float],
    hardness_hv: list[float],
    hardness_hrc: list[float | None],
    ...
    # Plot HV curve
    ax1.plot(distances_mm, hardness_hv, "b-o", linewidth=2, markersize=6, label="Hardness (HV)")
    ...
    if show_hrc:
        ...
        valid_hrc = [(d, h) for d, h in zip(distances_mm, hardness_hrc, strict=False) if h is not None]
        if valid_hrc:
            d_hrc, hrc_vals = zip(*valid_hrc, strict=False)
            ax2.plot(
                d_hrc,
                hrc_vals,
                "r--s",
...
def create_tornado_plot(
    sensitivity_data: dict, output_key: str = "t8_5", title: str = "Sensitivity Analysis"
) -> bytes:
    ...
    if not labels:
        ...
        ax.text(
            0.5,
            0.5,
            f"No sensitivity data for {output_label}",
            ha="center",
            va="center",
            transform=ax.transAxes,
            fontsize=14,
    ...
    total_impact = [abs(h - l) for h, l in zip(high_impacts, low_impacts, strict=False)]
    ...
    for i, idx in enumerate(sorted_idx):
        ax.barh(
            ...
            height=0.5,
            align="center",
            label="Increase" if i == len(sorted_idx) - 1 else None,
        )
        ax.barh(
            i,
            low_impacts[idx],
            ...
            height=0.5,
            align="center",
            label="Decrease" if i == len(sorted_idx) - 1 else None,
        )

        # Value annotations
        if high_impacts[idx] != 0:
            ax.text(
                high_impacts[idx] + 0.5,
                i,
                f"{high_impacts[idx]:+.1f}%",
                va="center",
                ha="left",
                fontsize=9,
            )
...
def create_spider_plot(
    sensitivity_data: dict, title: str = "Parameter Sensitivity Overview"
) -> bytes:
    ...
    for i, output_key in enumerate(output_keys):
        ...
        ax.plot(
            ...
            linewidth=2,
            label=OUTPUT_LABELS[output_key],
            markersize=5,
        )
...
def create_convergence_plot(opt_data: dict, title: str = "Optimization Convergence") -> bytes:
    ...
    running_best = []
    best = float("inf")
    for obj, feas in zip(obj_values, feasible, strict=False):
    ...
    feas_x = [x for x, f in zip(iters, feasible, strict=False) if f]
    feas_y = [y for y, f in zip(obj_values, feasible, strict=False) if f]
    infeas_x = [x for x, f in zip(iters, feasible, strict=False) if not f]
    infeas_y = [y for y, f in zip(obj_values, feasible, strict=False) if not f]

    if feas_x:
        ax.scatter(feas_x, feas_y, c="#2196F3", s=40, zorder=3, label="Feasible", alpha=0.7)
    if infeas_x:
        ax.scatter(
            infeas_x,
            infeas_y,
    ...
    buf = io.BytesIO()
    fig.savefig(
        buf, format="png", dpi=130, bbox_inches="tight", facecolor="white", edgecolor="none"
    )
    buf.seek(0)
    plt.close(fig)
    return buf.getvalue()
...
def create_peak_temperature_profile_plot(
    distances: list[float], peak_temps: list[float], zone_boundaries: dict | None = None
) -> bytes:
    ...
    for temp, label, color in ref_temps:
        ...
        ax.text(max(distances) * 0.95, temp + 20, label, ha="right", fontsize=8, color=color)
...
def create_hardness_traverse_plot(
    ...
    # Limit line
    ax.axhline(
        y=limit_hv, color="red", linestyle="--", linewidth=2, label=f"Limit = {limit_hv:.0f} HV"
    )
...
def create_haz_thermal_cycle_comparison_plot(
    thermal_cycles: dict, title: str = "HAZ Thermal Cycle Comparison"
) -> bytes:
...
def create_preheat_summary_plot(preheat_data: dict) -> bytes:
    ...
    for line in text_lines:
        if "Cracking Risk" in line:
            ...
            ax2.text(
                0.1, y_pos, line, transform=ax2.transAxes, fontsize=14, fontweight="bold", va="top"
            )
        else:
            ax2.text(0.1, y_pos, line, transform=ax2.transAxes, fontsize=11, va="top")
        y_pos -= 0.12
...
def create_parameter_trajectory_plot(opt_data: dict, title: str = "Parameter Trajectory") -> bytes:
...
def create_goldak_temperature_heatmap(goldak_data: dict) -> bytes:
    ...
    try:
        cs = ax.contour(y, z, peak_map, levels=contour_temps, colors=contour_colors, linewidths=1.5)
...
def create_goldak_rosenthal_comparison_plot(comparison_data: dict) -> bytes:
    ...
    ax2.bar(
        x_pos - bar_width / 2, goldak_vals, bar_width, label="Goldak", color="#2196F3", alpha=0.8
    )
    ax2.bar(
        x_pos + bar_width / 2, ros_vals, bar_width, label="Rosenthal", color="#FF5722", alpha=0.8
    )
...
def create_goldak_thermal_cycle_plot(thermal_cycles: dict, title: str = "") -> bytes:
    ...
    for idx, (name, cycle) in enumerate(thermal_cycles.items()):
        ...
        if times and temps:
            ...
            ax.plot(times, temps, "-", color=color, linewidth=1.5, label=label, alpha=0.9)

    ax.axhline(800, color="gray", linestyle=":", alpha=0.4, linewidth=1)
    ax.axhline(500, color="gray", linestyle=":", alpha=0.4, linewidth=1)
    ax.text(0.02, 810, "800\u00b0C", transform=ax.get_yaxis_transform(), fontsize=8, color="gray")
    ax.text(0.02, 510, "500\u00b0C", transform=ax.get_yaxis_transform(), fontsize=8, color="gray")

    ax.set_xlabel("Time (s)", fontsize=11)
    ax.set_ylabel("Temperature (\u00b0C)", fontsize=11)
    ax.set_title(
        title or "Goldak \u2014 Thermal Cycles at Probe Points", fontsize=13, fontweight="bold"
    )
...
def create_goldak_multipass_heatmap(multipass_data: dict) -> bytes:
    ...
    try:
        ...
        cs = ax.contour(
            ...
            z,
            peak_map,
            levels=contour_temps,
            colors=["white", "#FFD700", "#00FFFF", "#00FF00"],
            linewidths=1.5,
        )
        ax.clabel(cs, fmt={1500: "Solidus", 1100: "CGHAZ", 900: "Ac3", 727: "Ac1"}, fontsize=8)
    except ValueError:
        pass
    ...
    ax.set_ylabel("Depth z (mm)", fontsize=11)
    ax.set_title(
        f"Cumulative Peak Temperature \u2014 {n_passes} Passes", fontsize=13, fontweight="bold"
    )
    ax.invert_yaxis()
