...
class ValidationReportGenerator:
    ...
    def _add_simulation_summary(self):
        ...
        if ht.get("heating", {}).get("enabled"):
            ...
            rows.append(
                (
                    "Austenitizing",
                    f"{h.get('target_temperature', 850):.0f} °C, hold {h.get('hold_time', 0):.0f} min",
                )
            )
        ...
        if q:
            rows.append(
                (
                    "Quenching",
                    f"{q.get('media', 'water').title()}, {q.get('agitation', 'none')}, {q.get('media_temperature', 25):.0f} °C",
                )
            )
        if ht.get("tempering", {}).get("enabled"):
            ...
            rows.append(
                (
                    "Tempering",
                    f"{t.get('temperature', 550):.0f} °C, hold {t.get('hold_time', 60):.0f} min",
                )
            )
    ...
    def _add_measured_data_summary(self):
        ...
        for i, md in enumerate(measured_list, 1):
            ...
            table.rows[i].cells[4].text = (
                f"{md.duration_seconds:.0f}" if md.duration_seconds else "-"
            )
        self.doc.add_paragraph()
    ...
    def _add_conclusions(self):
        ...
        if metrics.time_offset:
            bullets.append(
                f"Detected time offset: {metrics.time_offset:.1f}s (auto-corrected in analysis)"
            )
        if metrics.phase_metrics:
            ...
            bullets.append(
                f"Best phase match: {best_phase[0].title()} (RMS {best_phase[1].get('rms', 0):.1f}°C)"
            )
            if best_phase[0] != worst_phase[0]:
                bullets.append(
                    f"Worst phase match: {worst_phase[0].title()} (RMS {worst_phase[1].get('rms', 0):.1f}°C)"
                )
...
class ValidationPDFReportGenerator:
    ...
    def _add_header(self):
        ...
        self.pdf.cell(
            0, 15, "Simulation Validation Report", align="C", new_x="LMARGIN", new_y="NEXT"
        )
        ...
        self.pdf.cell(
            0,
            6,
            f"Steel: {grade}  |  Geometry: {self.sim.geometry_label}  |  Date: {datetime.utcnow().strftime('%Y-%m-%d')}",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        self.pdf.ln(5)
    ...
    def _add_simulation_summary(self):
        ...
        if ht.get("heating", {}).get("enabled"):
            h = ht["heating"]
            lines.append(
                f"Austenitizing: {h.get('target_temperature', 850):.0f} C, hold {h.get('hold_time', 0):.0f} min"
            )
        q = ht.get("quenching", {})
        if q:
        ...
        if ht.get("tempering", {}).get("enabled"):
            t = ht["tempering"]
            lines.append(
                f"Tempering: {t.get('temperature', 550):.0f} C, hold {t.get('hold_time', 60):.0f} min"
            )
        for line in lines:
    ...
    def _add_measured_summary(self):
        ...
        for md in measured_list:
            ...
            self.pdf.cell(
                0,
                5,
                f"{md.name} | {(md.process_step or 'full').title()} | {md.num_channels} ch, {md.num_points} pts, {dur}",
                new_x="LMARGIN",
                new_y="NEXT",
            )
        self.pdf.ln(3)
    ...
    def _add_comparison_metrics(self):
        ...
        self.pdf.cell(
            col_w, 6, metrics.rating.title(), border=1, align="C", new_x="LMARGIN", new_y="NEXT"
        )
        self.pdf.ln(5)
        ...
        for md in measured_list:
            for ch in md.available_channels:
                ...
                plot_bytes = visualization.create_comparison_plot(
                    ...
                    sim_temps=sim_temps,
                    measured_data=[
                        {"name": f"{md.name} - {ch}", "times": meas_times, "temps": meas_temps}
                    ],
                    title=f"Sim vs {ch} ({md.process_step or 'full'})",
                )
    ...
    def _add_interpretation(self):
        ...
        if metrics.rating == "good":
            text = (
                f"The simulation shows good agreement (RMS {metrics.rms_error:.1f}C, "
                f"R2 {metrics.r_squared:.4f}). The model can be used with confidence."
            )
        elif metrics.rating == "acceptable":
            text = (
                f"Acceptable agreement (RMS {metrics.rms_error:.1f}C). "
                "Review boundary conditions and material properties for improvement."
            )
        else:
            text = (
                f"Poor agreement (RMS {metrics.rms_error:.1f}C). "
