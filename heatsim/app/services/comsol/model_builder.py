...
if TYPE_CHECKING:
    from app.models.weld_project import WeldProject, WeldString
...
class WeldModelBuilder:
    ...
    def _import_geometry(self, model: Any, project: "WeldProject") -> None:
        ...
        try:
            bodies = self.client.import_cad(model, project.cad_file, filename, format=cad_format)
    ...
    def _create_interpolation(
        ...
        try:
            ...
            # Build table data
            table_data = [[str(x), str(y)] for x, y in zip(x_data, y_data, strict=False)]
    ...
    def _setup_parameters(self, model: Any, project: "WeldProject") -> None:
        ...
        params = {
            ...
            "T_solidification": (
                project.default_solidification_temp,
    ...
    def activate_string(
        self, model: Any, string: "WeldString", prev_temps: dict[str, float] | None = None
    ) -> None:
    ...
    def calculate_initial_temp(
        self, string: "WeldString", prev_temps: dict[str, float] | None = None
    ) -> float:
    ...
    def update_study_time(
        self, model: Any, start_time: float, end_time: float, time_step: float = 1.0
    ) -> None:
