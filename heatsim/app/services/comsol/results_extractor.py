...
if TYPE_CHECKING:
    from app.models.weld_project import WeldResult, WeldString
...
class ResultsExtractor:
    ...
    def extract_string_results(
        self, model: Any, string: "WeldString", output_folder: Path
    ) -> list["WeldResult"]:
    ...
    def _extract_thermal_cycle(
        ...
        """
        from app.models.weld_project import RESULT_THERMAL_CYCLE, WeldResult
    ...
    def _extract_temperature_field(
        self, model: Any, string: "WeldString", output_folder: Path
    ) -> Optional["WeldResult"]:
        ...
        """
        from app.models.weld_project import RESULT_TEMPERATURE_FIELD, WeldResult
        ...
        try:
            ...
            self._export_vtk(
                model, str(vtk_filename), string.simulation_start_time + string.simulation_duration
            )
    ...
    def _extract_cooling_rate(
        ...
        WeldResult or None
            Cooling rate result
        """
        from app.models.weld_project import RESULT_COOLING_RATE, WeldResult
        ...
        try:
            ...
            avg_rate_800_500 = (
                float(np.mean(rates_cooling[range_mask])) if np.any(range_mask) else None
            )
    ...
    def _get_time_steps(self, model: Any) -> np.ndarray | None:
    ...
    def _evaluate_at_point(
        self, model: Any, point: tuple[float, float, float], times: np.ndarray
    ) -> np.ndarray | None:
    ...
    def _calculate_t8_5(self, times: np.ndarray, temps: np.ndarray) -> float | None:
    ...
    def _calculate_max_cooling_rate(self, times: np.ndarray, temps: np.ndarray) -> float | None:
    ...
    def _estimate_phases(self, t_800_500: float | None) -> dict:
    ...
    def extract_probe_data(
        self, model: Any, probe_points: list[tuple[float, float, float]], names: list[str] = None
        ...
        results = {}
        for name, point in zip(names, probe_points, strict=False):
    ...
    def extract_field_sequence(
        self, model: Any, times: list[float], output_folder: Path
    ) -> list[Path]:
