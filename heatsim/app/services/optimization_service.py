...
PARAMETER_DEFINITIONS = {
    ...
    "heating.hold_time": {
        "label": "Austenitizing Hold Time",
        "unit": "min",
        "default_min": 10,
        "default_max": 480,
    },
    "quenching.media_temperature": {
        "label": "Quench Media Temperature",
        "unit": "°C",
        "default_min": 0,
        "default_max": 200,
    },
    "quenching.duration": {
        "label": "Quench Duration",
        "unit": "s",
        "default_min": 30,
        "default_max": 7200,
    },
...
class OptimizationService:
    ...
    def optimize(
        ...
        try:
            if method == "differential_evolution":
                ...
                safe_popsize = max(
                    2, min(15, self.MAX_TOTAL_EVALUATIONS // (n_params * max_iterations))
                )
                logger.info(
    ...
    def _objective_function(
        ...
        if self._eval_count >= self.MAX_TOTAL_EVALUATIONS:
            raise _OptimizationTimeout(
                f"Evaluation count {self._eval_count} reached limit {self.MAX_TOTAL_EVALUATIONS}"
            )
        ...
        try:
            ...
            if diagram and grade.composition:
                ...
                try:
                    ...
                    outputs["martensite"] = round(center_phases.get("martensite", 0) * 100, 1)
                    outputs["bainite"] = round(center_phases.get("bainite", 0) * 100, 1)
                except Exception:
                    pass
    ...
    def _apply_params_to_config(
        self, key: str, value: float, ht_config: dict, geom_config: dict
    ) -> None:
    ...
    def _get_current_value(self, key: str) -> float | None:
