...
class ExcelImporter:
    ...
    def _import_grades(cls, xl: pd.ExcelFile, data_source: str, results: dict):
        ...
        for _, row in df.iterrows():
            ...
            existing = SteelGrade.query.filter_by(
                designation=designation, data_source=data_source
            ).first()
    ...
    def _import_properties(cls, xl: pd.ExcelFile, data_source: str, results: dict):
        ...
        for _, row in df.iterrows():
            ...
            existing = MaterialProperty.query.filter_by(
                steel_grade_id=grade.id, property_name=property_name
            ).first()
    ...
    def _import_diagrams(cls, xl: pd.ExcelFile, data_source: str, results: dict):
        ...
        for _, row in df.iterrows():
            ...
            # Create diagram
            diagram = PhaseDiagram(steel_grade_id=grade.id, diagram_type=diagram_type)
    ...
    @classmethod
    def export_to_excel(cls, grade_ids: list | None = None) -> bytes:
    ...
    def get_template(cls) -> bytes:
        ...
        ws_props.cell(
            row=3, column=6, value='{"temperature": [20, 200, 400], "value": [44, 42, 38]}'
        )
