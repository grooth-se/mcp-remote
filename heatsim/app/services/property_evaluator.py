...
class PropertyEvaluator:
    ...
    def evaluate(self, **conditions) -> float | None:
    ...
    def _eval_constant(self) -> float | None:
        """Evaluate constant property."""
        return self.data.get("value")

    def _eval_curve(self, **conditions) -> float | None:
        ...
        # Legacy user-entered data may contain quoted numbers — coerce defensively
        try:
    ...
    def _eval_table(self, **conditions) -> float | None:
    ...
    def _eval_polynomial(self, **conditions) -> float | None:
        """Evaluate polynomial.
    ...
    def _eval_equation(self, **conditions) -> float | None:
...
def evaluate_property(property_model, **conditions) -> float | None:
...
class PropertyPlotter:
    ...
    def plot_property(
        ...
        if prop_type == "constant":
            ...
            try:
                ...
                return self._create_empty_plot(
                    ax, fig, "No value stored for this constant property"
                )
            ...
            # Legacy user-entered data may contain quoted numbers — coerce defensively
            try:
                orig_temps = [float(t) for t in orig_temps]
                orig_values = [float(v) for v in orig_values]
            except (TypeError, ValueError):
                return self._create_empty_plot(
                    ax,
                    fig,
                    "Invalid curve data: non-numeric values.\nRe-save this property to fix it.",
                )
        ...
        ax.set_title(
            f"{property_model.display_name} vs Temperature", fontsize=12, fontweight="bold"
        )
    ...
    def plot_phase_properties(
        self, phase_properties: list, property_type: str = "density"
    ) -> bytes:
        ...
        if property_type == "density":
            ...
            ax.set_title(
                "Phase Relative Densities (Reference: Ferrite at 20°C = 1.0)",
                fontsize=12,
                fontweight="bold",
            )
        else:
            ax.set_ylabel("Thermal Expansion Coefficient (µm/m·K)", fontsize=11)
            ax.set_title("Phase Thermal Expansion Coefficients", fontsize=12, fontweight="bold")
    ...
    def plot_expansion_vs_temperature(
        self, phase_properties: list, temp_range: tuple = (0, 900), n_points: int = 100
    ) -> bytes:
        ...
        for pp in phase_properties:
            ...
            ax.plot(
                temps, values, color=color, linewidth=2, label=pp.phase_label, linestyle=linestyle
            )
