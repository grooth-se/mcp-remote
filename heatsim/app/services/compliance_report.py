...
class ComplianceReportGenerator:
    ...
    def _add_material_section(self):
        ...
        self.doc.add_paragraph(f"Designation: {self.snapshot.steel_grade_designation}")
        self.doc.add_paragraph(f"Data Source: {self.snapshot.steel_grade_data_source or 'N/A'}")
    ...
    def _add_results_section(self):
        ...
        metrics = [
            ...
            (
                "Duration",
                f"{self.snapshot.duration_seconds:.1f} s"
                if self.snapshot.duration_seconds
                else "N/A",
            ),
            ("t8/5", f"{self.snapshot.t_800_500:.2f} s" if self.snapshot.t_800_500 else "N/A"),
            (
                "Surface Hardness",
                f"{self.snapshot.predicted_hardness_surface:.0f} HV"
                if self.snapshot.predicted_hardness_surface
                else "N/A",
            ),
            (
                "Center Hardness",
                f"{self.snapshot.predicted_hardness_center:.0f} HV"
                if self.snapshot.predicted_hardness_center
                else "N/A",
            ),
        ]
    ...
    def _add_lineage_section(self):
        ...
        entries = [
            ...
            (
                "Started At",
                self.snapshot.started_at.strftime("%Y-%m-%d %H:%M:%S")
                if self.snapshot.started_at
                else "N/A",
            ),
            (
                "Completed At",
                self.snapshot.completed_at.strftime("%Y-%m-%d %H:%M:%S")
                if self.snapshot.completed_at
                else "N/A",
            ),
        ]
        ...
        if drifts:
            ...
            self.doc.add_paragraph(
                "No material drift detected. Frozen data matches current database."
            )
    ...
    def _add_change_log_section(self):
        ...
        changes = (
            MaterialChangeLog.query.filter(
