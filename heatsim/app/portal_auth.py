...
def _ensure_local_user(portal_user):
    ...
    try:
        ...
        portal_role = portal_user.get("role") or ("admin" if portal_user.get("is_admin") else None)

        user = User.query.filter_by(username=username).first()
        if not user:
            user = User(username=username)
            for field in ("display_name", "full_name", "email"):
                if hasattr(user, field) and portal_user.get("display_name"):
                    try:
                        setattr(user, field, portal_user.get("display_name"))
                    except (AttributeError, TypeError):
                        pass
            # Set admin/role — skip if property is read-only
            for attr, val in [
