...
from app.models import DATA_SOURCE_STANDARD, SteelGrade
...
class TestEditJMAK:
    ...
    def test_edit_jmak_get(self, logged_in_client, grade_with_ttt):
        resp = logged_in_client.get(f"/ttt-cct/grade/{grade_with_ttt.id}/jmak/pearlite")
        assert resp.status_code == 200
...
class TestEditMartensite:
    def test_edit_martensite_get(self, logged_in_client, grade_with_ttt):
        resp = logged_in_client.get(f"/ttt-cct/grade/{grade_with_ttt.id}/martensite")
        assert resp.status_code == 200
