...
class TestTemplateEdit:
    ...
    def test_updates(self, logged_in_client, sample_template, db):
        rv = logged_in_client.post(
            f"/templates/{sample_template.id}/edit",
            data={
                "name": "Updated Name",
    ...
    def test_config_saves(self, logged_in_client, sample_template, db):
        rv = logged_in_client.post(
            f"/templates/{sample_template.id}/config",
            data={
                "heating_enabled": "on",
...
class TestTemplateDuplicate:
    def test_success(self, logged_in_client, sample_template, db):
        rv = logged_in_client.post(
            f"/templates/{sample_template.id}/duplicate", follow_redirects=True
        )
        assert rv.status_code == 200
        copy = HeatTreatmentTemplate.query.filter(
            HeatTreatmentTemplate.name.contains("(Copy)")
        ).first()
    ...
    def test_public_ok(self, logged_in_client, db, admin_user):
        ...
        db.session.add(tmpl)
        db.session.commit()
        rv = logged_in_client.post(f"/templates/{tmpl.id}/duplicate", follow_redirects=True)
        assert rv.status_code == 200
...
class TestTemplateApply:
    def test_success(self, logged_in_client, sample_template, sample_simulation, db):
        rv = logged_in_client.post(
            f"/templates/{sample_template.id}/apply/{sample_simulation.id}", follow_redirects=True
        )
        assert rv.status_code == 200
    ...
    def test_increments_use_count(self, logged_in_client, sample_template, sample_simulation, db):
        ...
        logged_in_client.post(f"/templates/{sample_template.id}/apply/{sample_simulation.id}")
        tmpl = db.session.get(HeatTreatmentTemplate, sample_template.id)
    ...
    def test_access_denied_template(self, logged_in_client, sample_simulation, db, admin_user):
        ...
        db.session.commit()
        rv = logged_in_client.post(
            f"/templates/{tmpl.id}/apply/{sample_simulation.id}", follow_redirects=True
        )
        assert rv.status_code == 200  # Redirect

    def test_access_denied_sim(
        self, logged_in_client, sample_template, db, sample_steel_grade, admin_user
    ):
        sim = Simulation(
            name="Other Sim",
        ...
        db.session.commit()
        rv = logged_in_client.post(
            f"/templates/{sample_template.id}/apply/{sim.id}", follow_redirects=True
        )
        assert rv.status_code == 200  # Redirect
