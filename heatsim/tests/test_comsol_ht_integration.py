...
from app.models.simulation import (
    ...
    STATUS_DRAFT, STATUS_RUNNING, STATUS_COMPLETED, STATUS_FAILED,
    GEOMETRY_CYLINDER,
)
from app.models.snapshot import SimulationSnapshot
from app.models import SteelGrade, User, ROLE_ENGINEER, DATA_SOURCE_STANDARD
from app.models.material import SteelComposition, PhaseDiagram, MaterialProperty
from app.services.comsol import (
    MockHeatTreatmentSolver, HeatTreatmentResultsExtractor,
...
class TestResultsExtractorDB:
    ...
    def _create_sim(self, db_session):
        ...
        sim.set_ht_config({
            'heating': {
                'enabled': True, 'target_temperature': 850.0,
                'hold_time': 60.0, 'initial_temperature': 25.0,
                'h_conv': 50.0, 'cold_furnace': False,
            },
            'transfer': {
                'enabled': True, 'duration': 10.0,
                'ambient_temperature': 25.0, 'h_conv': 10.0,
            },
            'quenching': {
                'enabled': True, 'media': 'oil',
                'media_temperature': 60.0, 'duration': 300.0, 'h_conv': 500.0,
            },
...
class TestFallbackChain:
    ...
    def _create_runnable_sim(self, db_session):
        ...
        sim.set_ht_config({
            'heating': {
                'enabled': True, 'target_temperature': 850.0,
                'hold_time': 10.0, 'initial_temperature': 25.0,
                'cold_furnace': False, 'h_conv': 50.0,
                'furnace_ramp_rate': 0,
            },
            'transfer': {
                'enabled': True, 'duration': 5.0,
                'ambient_temperature': 25.0, 'h_conv': 10.0,
            },
            'quenching': {
                'enabled': True, 'media': 'water',
                'media_temperature': 25.0, 'duration': 60.0, 'h_conv': 1500.0,
            },
            'tempering': {'enabled': False},
        })
        db_session.add(sim)
...
class TestSharedHelpers:
    ...
    def _create_sim_with_composition(self, db_session):
        ...
        comp = SteelComposition(
            steel_grade_id=grade.id,
            carbon=0.40, silicon=0.23, manganese=0.70, chromium=0.80,
            nickel=1.83, molybdenum=0.25, vanadium=0.0,
        )
        db_session.add(comp)
        ...
        snapshot = SimulationSnapshot(
            simulation_id=sim.id, version=1,
            status='running',
    ...
    def test_predict_phases_no_diagram_no_composition(self, app, db):
        with app.app_context():
            ...
            from app.services.simulation_runner import _predict_phases
            tracker, phases = _predict_phases(
                sim, snapshot, grade, None,
                np.linspace(0, 600, 100), np.linspace(850, 25, 100), 15.0
            )
    ...
    def test_predict_hardness_no_grade(self, app, db):
        ...
        with app.app_context():
            ...
            _predict_hardness(
                sim, snapshot, grade,
                {'tempering': {'enabled': False}},
                np.zeros((100, 2)), np.linspace(0, 600, 100), None
            )
