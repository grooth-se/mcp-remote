...
def test_validate_token_success(client, app, admin_user):
    ...
    resp = client.post(
        "/api/validate-token", data=json.dumps({"token": token}), content_type="application/json"
    )
    data = resp.get_json()
...
def test_validate_token_missing(client, app):
    resp = client.post("/api/validate-token", data=json.dumps({}), content_type="application/json")
