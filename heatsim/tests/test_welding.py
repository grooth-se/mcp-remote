...
class TestWeldingIndex:
    ...
    def test_shows_own_only(
        self, logged_in_client, sample_weld_project, db, sample_steel_grade, admin_user
    ):
        other = WeldProject(
            name="Admin Weld",
...
class TestWeldingConfigure:
    ...
    def test_quick_add(self, logged_in_client, sample_weld_project, db):
        rv = logged_in_client.post(
            f"/welding/{sample_weld_project.id}/configure",
            data={
                "action": "quick_add",
                "num_layers": "2",
                "strings_per_layer": "1",
...
class TestWeldStrings:
    ...
    def test_creates_string(self, logged_in_client, sample_weld_project, db):
        rv = logged_in_client.post(
            f"/welding/{sample_weld_project.id}/string/new",
            data={
                "string_number": "1",
                "name": "Root Pass",
                "body_name": "",
    ...
    def test_delete_string(self, logged_in_client, sample_weld_project, db):
        ...
        db.session.commit()
        rv = logged_in_client.post(
            f"/welding/{sample_weld_project.id}/string/{ws.id}/delete", follow_redirects=True
        )
        assert rv.status_code == 200
...
class TestWeldingDuplicate:
    def test_success(self, logged_in_client, sample_weld_project, db):
        rv = logged_in_client.post(
            f"/welding/{sample_weld_project.id}/duplicate", follow_redirects=True
        )
        assert rv.status_code == 200
...
class TestWeldingPlots:
    def test_invalid_type_404(self, logged_in_client, sample_weld_project):
        rv = logged_in_client.get(f"/welding/{sample_weld_project.id}/plot/nonexistent_type")
...
class TestGoldakMultipassQueue:
    ...
    def test_status_endpoint(self, logged_in_client, sample_weld_project, db):
        ...
        rv = logged_in_client.get(f"/welding/{sample_weld_project.id}/goldak/multipass/status")
        assert rv.status_code == 200
        data = rv.get_json()
