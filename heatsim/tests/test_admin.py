...
from app.models import ROLE_ADMIN, ROLE_ENGINEER, AuditLog, SystemSetting, User
...
class TestEditUser:
    ...
    def test_change_role(self, admin_client, engineer_user, db):
        rv = admin_client.post(
            f"/admin/users/{engineer_user.id}/edit",
            data={
                "role": ROLE_ADMIN,
            },
            follow_redirects=True,
...
class TestSettings:
    ...
    def test_maintenance_mode_blocks_engineers(self, client, db, admin_user, engineer_user):
        ...
        client.post("/auth/login", data={"username": "admin1", "password": "adminpass"})
        client.post(
            "/admin/settings",
            data={
                "maintenance_mode": "on",
        ...
        # Login as engineer
        client.post("/auth/login", data={"username": "engineer1", "password": "password123"})
        rv = client.get("/")
