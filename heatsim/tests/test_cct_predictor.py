            ...
            for curve_type in ("start", "finish"):
                for point in data[curve_type]:
                    ...
                    assert isinstance(point[0], float), (
                        f"{phase} {curve_type}: time should be float"
                    )
                    assert isinstance(point[1], float), (
                        f"{phase} {curve_type}: temp should be float"
                    )
        ...
        for phase, data in curves.items():
            for curve_type in ("start", "finish"):
                for time, temp in data[curve_type]:
                    assert 150 < temp < 950, f"{phase} {curve_type}: temp {temp}°C out of range"
...
class TestHardenabilityEffects:
    def test_higher_alloy_shifts_right(self, plain_carbon_comp, aisi4340_comp):
        ...
        assert alloy_nose[0] > plain_nose[0], "Alloyed steel should have longer incubation times"
